...
Singleton {
    ...
    component BrightnessMonitor: QtObject {
        ...
        property real clampedBrightness: Math.max(0, Math.min(1, brightness))
        ...
        function syncBrightness() {
            const brightnessValue = Math.max(monitor.clampedBrightness, 0);
            if (isDdc) {
